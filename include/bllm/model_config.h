...
struct ModelConfig {
  ...
  std::string arch;                              // "hybrid" (Qwen3.5) | "dense" (KV models)
  std::string hbm;                               // resolved absolute path to the .hbm
  std::string tokenizer;                         // resolved path to tokenizer.json
  std::string embed;                             // resolved path to fp16 embed table (hybrid only)
  std::string graph = "qwen35";                  // graph name inside the .hbm (hybrid)
  std::vector<int> eos;                          // stop-token ids
  int cache_len = 0;                             // informational
  double rope_theta = 1e7;                       // hybrid rope base
  ChatConfig chat;
