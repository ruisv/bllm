...
| `token_bytes(token_id) -> bytes` | The exact bytes a token contributes (empty for control tokens). `decode([id])` cannot supply them: a token holding part of a multi-byte character decodes to U+FFFD, which corrupts any per-token reassembly of CJK — the reason OpenAI's `bytes` field exists. |
