...
The compiled package is on Hugging Face —
**[ruisv/bllm-pi05-libero-224-s100p](https://huggingface.co/ruisv/bllm-pi05-libero-224-s100p)**
(5.3 GB: three `.hbm`, the embedding table, tokenizer, and the manifest).

```bash
huggingface-cli download ruisv/bllm-pi05-libero-224-s100p \
    --local-dir pi05-libero-224-s100p
```

These are derived weights — openpi's π0.5 checkpoint compiled for the BPU, with
Google's PaliGemma tokenizer. See the model card for provenance and upstream
terms.
...
There is also a C++ entry point, `examples/pi05_policy.cc`, which does the same
thing from the shell. It is an example rather than a shipped binary — the conda
package builds with `BLLM_BUILD_EXAMPLES=OFF` — so it needs a source build:

```bash
cmake -S . -B build -DBLLM_BUILD_EXAMPLES=ON && cmake --build build
./build/examples/bllm_pi05_policy --model pi05-libero-224-s100p \
