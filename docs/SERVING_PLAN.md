...
## 容量与预算实测（瘦身后，2026-07-20）

瘦身后在 3 GB 预算下实填（Qwen3.5-2B ctx4k，每段两轮对话）：
...
填到 26 条 / 785 MB 时 0 次淘汰，容量随预算线性增长。

**但 >2 GB 的真实约束不是缓存本身，而是整机 RAM。** 测试时容器（2 GB 预算）与
测试服务（3 GB）并存，`MemAvailable` 掉到 2.4 GB —— 两个进程各自都"合理"，加起来超了。
这正是固定默认值的问题：**没有哪个 MB 数在所有部署下都对**。

所以 `BLLM_CACHE_MAX_MB` 默认改为 `auto`：按启动时 `MemAvailable` 的 40% 取值
（下限 256 MB、上限 4 GB），并把选定值打进日志。显式值仍然优先，`0` 仍然关闭。
取少数份额是刻意的 —— 8 GB 板子上超配不会抛异常，而是把板子搞死（本项目开发期间发生过）。
...
- ~~**缓存预算自适应**~~ —— **已完成**：`BLLM_CACHE_MAX_MB` 默认改为 `auto`，
  按 `MemAvailable` 的 40% 取值（下限 256 MB、上限 4 GB），启动时打印选定值。
- ~~**hybrid 快照瘦身**~~ —— **已完成**，见下节。
